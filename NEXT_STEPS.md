...
7. Design the first manual-review workflow for `overlap_with_changes` uploads, using the new changed-overlap review details as the starting point.
8. Decide how corrected re-uploads should be approved later without silent replacement.
9. Define the correct subset of Order Listing transaction families for future business sales truth.
10. Decide the final treatment of cancelled, memo, complimentary, sales return, and Part Payment rows in business totals.
11. Design a separate reliable payment-settlement truth layer for cash, card, due, and other split methods.
12. Promote the read-only sales-policy layer into dashboard and profit logic only after the policy is approved.
13. Finalize whether memo rows should remain unresolved, be excluded, or be included under a later approved rule.
14. If memo review hints are revisited later, require an approved linking rule instead of relying on heuristic candidates alone.
15. Treat weak memo evidence as low-confidence investigation only, not believable linkage.
16. Promote the policy only after month-level and upload-level reconciliation remain clean.
17. Add safer validation for spreadsheet structure before import.
18. Add stronger success and failure messages for uploads and imports.
19. Keep project docs updated whenever a new page, table, or flow is added.
20. If future review rules are added, put them into the reusable read-only policy layer first and protect them with invariant tests before any UI or live-metric promotion.
