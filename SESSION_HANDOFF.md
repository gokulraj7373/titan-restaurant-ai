...
- Added the first reusable item-level sales summary helper under `lib/sales-query/item-sales-summary.ts`.
- Extracted the Sales Analytics summary cards to use that helper for `Imported Rows`, `Item Revenue Total`, `Total Quantity`, and `Unique Bills`.
- Kept the helper narrow and deterministic by using `sales_item_imports` only.
- Kept order-level helpers separate and avoided any dependency on `lib/sales-truth-review/*`.
...
- Keep Titan in a read-only truth-review phase while extending the first small reusable live sales-query layer across order-level, expense-side, and item-level helpers.
