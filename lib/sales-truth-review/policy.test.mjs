...
test("identity mismatch prevents memo hint confidence from being overstated", () => {
  const memoRow = createRow({
    id: 41,
    order_no: "Memo",
    bill_date: "2026-03-10",
  ...
  const weakLaterCandidate = createRow({
    id: 42,
    order_no: "7001",
    bill_date: "2026-03-11",
    effective_total: 250,
  ...
  const candidates = buildMemoMatchCandidates(memoRow, [weakLaterCandidate]);

  assert.equal(candidates.length, 0);

  const weakAmountDateOnlyCandidates = buildMemoMatchCandidates(
    createRow({
      id: 43,
      order_no: "Memo",
      bill_date: "2026-03-10",
  ...
  assert.equal(weakAmountDateOnlyCandidates.length, 1);
  assert.equal(weakAmountDateOnlyCandidates[0].confidence, "low");
  assert.doesNotMatch(weakAmountDateOnlyCandidates[0].reasons.join(" "), /approved link/i);
});

test("memo review helper wording stays conservative in policy meaning", () => {
  const notesText = getMemoResolutionReviewNotes().join(" ").toLowerCase();
  const memoLabelText = getMemoReviewLabelText("cancelled_signal_review_only").toLowerCase();
  const bucketText = getSalesPolicyBucketText("unresolved_memo").toLowerCase();

  assert.match(notesText, /unresolved/);
  assert.match(notesText, /excluded from live sales truth/);
  assert.match(notesText, /heuristic/);
  assert.match(notesText, /approved/);
  assert.match(memoLabelText, /review only/);
  assert.match(bucketText, /unresolved memo/);
});
